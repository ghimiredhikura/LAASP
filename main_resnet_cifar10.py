...
def main():
    ...
    if args.mode == 'train':
        ...
        if os.path.isfile(args.pruned_path):
            ...
            flops_pruned = get_n_flops_(model_pruned, img_size=(32, 32))
...
def prune(model, train_loader, loader_subset, test_loader, criterion, log):
    ...
    filter_prune_limit_per_layer = 0.55

    model.eval()

    conv_filter_prune_idx_all = get_all_conv_index_to_prune(model)
    print("conv_filter_prune_idx_all", conv_filter_prune_idx_all)
    layers_step_prune_count = calc_conv_layers_step_prune_count(model, conv_filter_prune_idx_all)
...
def calc_conv_layers_step_prune_count(model, conv_filter_prune_idx):
