...
def main():
    ...
    if args.dataset == 'cifar10':
        dataset_train = CIFAR10(args.data_path, train=True, transform=transform_train, download=False)
        dataset_test = CIFAR10(args.data_path, train=False, transform=transform_test, download=False)
    elif args.dataset == 'cifar100':
    ...
    if args.mode == 'train':
        ...
        if os.path.isfile(args.pretrain_path):
            ...
            flops_pretrain = get_n_flops_(model, img_size=(32, 32))
            print_log("Pretrain Model Flops: %lf" % flops_pretrain, log)
        ...
        if os.path.isfile(args.pruned_path):
            print_log("Load model from %s" % (args.pruned_path), log)
...
def get_stats(net, in_w, in_h):
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(net, (3, in_w, in_h), as_strings=True,
                                                print_per_layer_stat=False, verbose=False)

        flops = float(macs.split()[0]) * 2 # macs to flops 
